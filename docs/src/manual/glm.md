...
- Model: ``y_i = \beta^T x_i + \epsilon_i``, ``\epsilon_i \sim \text{Normal}(0, \sigma^2)``
- Negative log-likelihood:
  ``\sum_i \left[ -y_i(x_i^T \beta) + \tfrac{1}{2}(x_i^T \beta)^2 \right]``
...
- Model: ``y_i \sim \text{Bernoulli}(\mu_i)``, ``\mu_i = \frac{1}{1+\exp(-x_i^T \beta)}``
- Negative log-likelihood:
  ``\sum_i \left[ -y_i(x_i^T \beta) + \log(1+\exp(x_i^T \beta)) \right]``

## Binomial Regression
Like logistic regression, but each observation records ``k`` successes
out of ``n`` trials, rather than a single 0/1 outcome.
- Model: ``y_i \sim \text{Binomial}(n_i, \mu_i)``, same logit link as logistic
- Negative log-likelihood:
  ``\sum_i \left[ -k_i(x_i^T \beta) + n_i \log(1+\exp(x_i^T \beta)) \right]``

## Poisson Regression
Predicts count data (non-negative integers), such as the number of
events occurring in a fixed interval.
- Model: ``y_i \sim \text{Poisson}(\mu_i)``, ``\mu_i = \exp(x_i^T \beta)``
- Negative log-likelihood:
  ``\sum_i \left[ -y_i(x_i^T \beta) + \exp(x_i^T \beta) \right]``

## Exponential Regression
Predicts a positive continuous value, commonly time until an event
occurs, assuming a constant event rate.
- Model: ``y_i \sim \text{Exponential}(\text{rate} = -x_i^T \beta)``, requires
  ``x_i^T \beta \leq 0``
- Negative log-likelihood:
  ``\sum_i \left[ -y_i(x_i^T \beta) - \log(-x_i^T \beta) \right]``