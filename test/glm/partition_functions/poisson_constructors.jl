module TestPoissonConstructor

using Test, OptimizationProblems

family = :Poisson
types = [Float16, Float32, Float64]
constructor = PoissonRegression
response_type(type) = Vector{Int64}
feature_type(type) = Matrix{type}

@testset "Poisson Regression Constructors" begin
    ...
    for type in types
        ...
        #Number of parameters should be at least one 
        let resp=round.(Int64, 10*rand(10)), feat=zeros(type, 10, 0) 
            @test_throws ArgumentError constructor(resp=resp, feat=feat)
        end

        #Number of observations should be at least one 
        let resp=round.(Int64, 10*rand(0)), feat=zeros(type, 0, 3)
            @test_throws ArgumentError constructor(resp=resp, feat=feat)
        end

        #Number of responses and number of observations in feature must match
        let resp=round.(Int64, 10*rand(10)), feat=zeros(type, 5, 3)
