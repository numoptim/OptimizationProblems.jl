...
response_type(type) = BitVector
feature_type(type) = Matrix{type}


@testset "Bernoulli Generalized Linear Model" begin
    ...
    for type in types
        ...
        #Verify sizes and types 
        let resp=(rand(1) .> 0.5), feat=zeros(type, 1, 3)
            problem = constructor(resp=resp, feat=feat)
        ...
        #Verify sizes and types 
        let resp=(rand(10) .> 0.5), feat=zeros(type, 10, 3)
            problem = constructor(resp=resp, feat=feat)
