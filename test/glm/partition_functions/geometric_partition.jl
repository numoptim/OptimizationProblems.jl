module TestGeometricPartition

using Test, OptimizationProblems, LinearAlgebra, Random

family = :Geometric
types = [Float16, Float32, Float64]
responses(type) = round.(Int64, rand(3) .* 5)
xs(type) = [-rand(type, 5), -rand(type, 10), -rand(type, 15)]
feats(type) = [rand(type, 5), rand(type, 10), rand(type, 15)]
logpartition(η) = -log(1 - exp(η))
derivative_lp(η) = exp(η) / (1 - exp(η))
second_der_lp(η) = exp(η) / (1 - exp(η))^2

@testset "Geometric Log Partition Function" begin

    for type in types
        for (x, resp, feat) in Iterators.zip(xs(type), responses(type),
            feats(type))

            # Helper: Linear Effect (clamp as in exponential_partition.jl to
            # handle floating-point edge cases in reference computations)
            ...
            let hessian=zeros(type, num_param, num_param), resp=resp, x=x,
                ...
                # Solution should be updated to hessian
