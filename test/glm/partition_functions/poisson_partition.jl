module TestPoissonPartition 

using Test, OptimizationProblems, LinearAlgebra, Random 

family = :Poisson 
types = [Float16, Float32, Float64]
responses(type) = round.(Int64, rand(3).*15)
xs(type) = [randn(type, 5), randn(type, 10), randn(type, 15)]
feats(type) = [randn(type, 5)./5, randn(type, 10)./10, randn(type, 15)./15]
logpartition(η) = exp(η)
derivative_lp(η) = exp(η)
second_der_lp(η) = exp(η)

@testset "Poisson Log Partition Function" begin 
