module TestGeometricConstructor

using Test, OptimizationProblems

family = :Geometric
types = [Float16, Float32, Float64]
constructor = GeometricRegression
response_type(type) = Vector{Int64}
feature_type(type) = Matrix{type}

@testset "Geometric Regression Constructors" begin
    ...
    for type in types
        ...
        let num_param=1, num_obs=1
            problem = constructor(type, num_param=num_param, num_obs=num_obs)
    ...
    for type in types
        ...
        # Verify sizes and types
        let resp=zeros(Int64, 1), feat=zeros(type, 1, 3)
            problem = constructor(resp=resp, feat=feat)
        ...
        # Verify sizes and types
        let resp=zeros(Int64, 10), feat=zeros(type, 10, 3)
            problem = constructor(resp=resp, feat=feat)
