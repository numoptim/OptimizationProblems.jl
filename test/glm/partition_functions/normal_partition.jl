    ...
    for type in types
        ...
        for (x, feat) in Iterators.zip(xs(type), feats(type))
            ...
            let resp=resp, x=x, feat=feat
                ...
                @test isapprox(val, resp^2/2 - resp*η + logpartition(η), 
                    atol=sqrt(eps(η))) # Needs Numerical Analysis
            end
